...
const response = await fetch('https://api.matterai.so/v1/chat/completions', {
  ...
  headers: {
    ...
    'Authorization': 'Bearer MATTERAI_API_KEY'
    'X-AXON-REASONING-EFFORT': 'high', // Optional header | 'none', 'low', 'medium', 'high', default is 'high'
    'X-AXON-REASONING-SUMMARY': 'none' // Optional header | 'none', 'auto', default is 'none'
  },
  body: JSON.stringify({
    model: 'axon',
    messages: [
